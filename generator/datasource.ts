...
export class XmlDataSource {
  ...
  private readEnumValueDefs(xml: any) {
    return xml.entry.map((xml: any) => ({
      ...
      name: xml.$.name,
      value: xml.$.value,
      description: xml.description?.map((s: any) => String(s))
        .filter((s: any) => s.trim() !== '[object Object]')
        .join(' ') || '',
  ...
  private readMessageDefs(mavlink: any): MessageDef[] & Pipeable<MessageDef[]> {
    const isWorkInProgress = (message: any) => message.$$.some((item: any) => item['#name'] === 'wip')

    const result = mavlink.messages[0].message.map((message: any) => ({
      source: {
        xml: message,
        name: message.$.name,
      },
    ...
    return pipeable(result)
      ...
      .pipe((messages: any) => messages.map((message: any) => ({
        ...message,
        payloadLength: message.fields.reduce((acc: any, field: any) => acc + field.size, 0),
      })))

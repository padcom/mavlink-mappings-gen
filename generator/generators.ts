...
export function generateMessageFields(output: Writer, fields: {
  name: string
  type: string
  description: string[]
  units: string
...
export function generateMagicNumbers(magicNumbers: Record<string, number>) {
  return [
...
function processMessages(output: Writer, messageDefs: MessageDef[] & Pipeable<MessageDef[]>) {
  const FIXED_MESSAGE_NAMES: Record<string, string> = {
    '111': 'TimeSync',
